  ...
  <section class="flex flex-col items-center text-center">
    ...
    <section class="w-full pt-16">
      <div class="relative">
        ...
        >
          <button
            class="text-xs h-full w-full outline-none button-medio"
            :class="buttonGenero === 'ciudadania' ? 'activateButton' : false"
            @click="setGender('ciudadania')"
          >
            Cédula ciudadania
          </button>
          <button
            class="text-xs h-full w-full outline-none"
            :class="buttonGenero === 'extranjera' ? 'activateButton' : false"
            @click="setGender('extranjera')"
          >
            Cédula Extranjera
          </button>
        </article>
        <IconError
          v-if="errores.tipoCedula"
          class="w-6 h-6 absolute icono-error2"
        />
      </div>
      <article class="relative mt-12">
        <input
          ...
          :class="{ inputError: errores.cedula && errores.cedula.error }"
          @click="activate('cedula')"
          @focus="activate('cedula')"
        ...
        <div
          ...
          :class="
            active === 'cedula' || data.cedula != '' ? 'moveLabel' : false
          "
        >
          <label for="" class="text-rojo text-xs" @click="activate('cedula')"
      ...
      <article class="relative my-12">
        <input
          :value="data.fecha"
          type="date"
          class="border input-fecha border-rojo rounded-md w-64 h-12 outline-none pl-4"
          :class="{ inputError: errores.fecha && errores.fecha.error }"
          @click="activate('fecha')"
          @focus="activate('fecha')"
          @input="(event) => inputValue(event, 'fecha')"
        />
        <IconError v-if="errores.fecha" class="w-6 h-6 absolute icono-error" />
        <div
          class="div-label absolute px-2"
          :class="active === 'fecha' || data.fecha != '' ? 'moveLabel' : false"
        >
          <label for="" class="text-rojo text-xs" @click="activate('fecha')"
