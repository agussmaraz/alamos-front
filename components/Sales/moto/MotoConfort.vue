<template>
    <section>
        <article class="overflow-y-auto py-48">
            <div v-for="(item, index) in filterName" :key="index" class="checkbox div-checkout px-3">
                <input :id="item.model" v-model="item.status" type="checkbox" :name="item.model" class="input-div. w-4 h-5 md:h-6" @change="change(item)" />
        ...
        data() {
            return {
                array: [
                    ...
                    { name: 'Sillín acolchado', model: 'sillin', status: false },
                    { name: 'Tablero digital', model: 'tablero', status: false },
                ],
                activateButton: '',
            };
        },
