<template>
  <div class="flex flex-col items-center md:ml-2 lg:mr-8">
    <div
      class="perfil__edit-photo flex flex-col items-center md:justify-center"
      :class="{ 'perfil__edit-photo-large': camera }"
    >
      <div
        v-if="!camera"
        class="perfil__new-image flex flex-col items-center justify-center cursor-pointer"
        @click="triggerUpload"
      >
        <div>
        ...
        <div class="mt-3 text-center texto">
          ...
          <input
            ...
            accept="image/*"
            style="display: none"
            @change="pickFile('profilePic')"
          />
        </div>
      </div>
      <hr />
      <div
        class="perfil__new-image flex flex-col justify-center items-center pb-6 cursor-pointer relative"
        :class="{ 'perfil__edit-image-large': camera }"
        @click="showCamera"
      >
        <div
          ref="camera"
          class="profile--camera max-w-full max-h-full absolute flex flex-col items-center mb-8"
        >
          <video ref="video" playsinline autoplay class="pb-4"></video>
          <button v-if="camera" id="startbutton" @click="takePic">
            Sacar foto
          </button>
        ...
        <div class="mt-3 text-center">
          <h2 v-if="!camera" class="text-base texto">
            Tomar una nueva fotografia
          </h2>
        </div>
      </div>
    </div>
    <div class="flex w-full justify-around mt-4 md:justify-center">
      <button class="font-semibold w-32 md:text-sm" @click="triggerCancel">
        Cancelar
      </button>
      <button
        :disabled="preview_image === null"
        class="perfil__form-button h-8 rounded-md font-semibold text-sm my-4"
        :class="{ disabled: preview_image === null }"
        @click="sendPhoto()"
      >
        Confirmar
      </button>
