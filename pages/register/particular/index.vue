<template>
    <section>
        ...
        <article class="flex justify-center">
            <div v-if="componente_actual !== 'DatosUser8'" class="bg-rojo w-48 py-2 text-center rounded-3xl shadow-lg mr-2">
                <button class="a" @click="changePage">Siguiente</button>
            </div>
            <div v-if="componente_actual === 'DatosUser8'" class="bg-rojo w-48 py-2 text-center rounded-3xl shadow-lg mr-2">
                <button class="a" @click="openModal({ modal: 'documento' })">¡Ya lo tengo!</button>
            </div>
        </article>
        <section v-if="modalDocumento" class="absolute inset-0 flex justify-center">
            <article class="modal z-10" @click="closeModal({ modal: 'documento' })"></article>
