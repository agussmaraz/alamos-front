...
export const actions = {
  ...
  validateComponente1({ state, dispatch }) {
    ...
    const confirmacionValidacion = validaciones.emailConfirmacion(
      state.datos.correo,
  ...
  validateComponente5({ state, dispatch }) {
    ...
    const repetirContraseña = validaciones.repetirContraseña(
      state.datos.contraseña,
      state.datos.repetirContraseña
    )
    if (contraseña !== true) {
  ...
  register({ commit }, data) {
    ...
    console.log(objeto)
    axios
      .post('https://auth.alamosautos.co/users/register', objeto)
